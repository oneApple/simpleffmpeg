		...
		if (ist->decoding_needed) {
			...
			case AVMEDIA_TYPE_VIDEO:
				...
				ret = avcodec_decode_video2(ist->st->codec,
						&picture, &got_picture, &avpkt);  //YUV存放在picture中
...
//					{
//						printf("%c\n",frame_type[index]);
//						for(hi = 0; hi < frame_height;++ hi)
//						{
//							printf("line:%d\n",hi);
//							for(wi = 0; wi < picture.linesize[index] - 32;++ wi)
//								printf("%d,",picture.data[index][hi * picture.linesize[index] + wi]);
//							printf("\n");
//						}
//					}
//				}
		...
		AVStreamMap *stream_maps, int nb_stream_maps)
{
	...
	file_table= av_mallocz(nb_input_files * sizeof(AVInputFile));  //分配空间
	if (!file_table)
		goto fail;
	...
	for(i=0;i<nb_input_files;i++) {
		is = input_files[i];
		if(!gfilename && is->filename)
		{
			gfilename = is->filename;
		}
...
#define METADATA_CHECK_INDEX(index, nb_elems, desc)\
	...
	/* dump the file output parameters - cannot be done before in case
       of stream copy */
	for(i=0;i<nb_output_files;i++) {
		dump_format(output_files[i], i, output_files[i]->filename, 1);   //在控制台输出两个流的相关信息
	}
	...
	for(; received_sigterm == 0;) {
		...
		if (output_packet(ist, ist_index, ost_table, nb_ostreams, &pkt) < 0) {  //这里面包含解码部分
...
static void opt_input_file(const char *filename)
{
	...
	if (last_asked_format) {  //the format is probe in av_open_input_file(),这里没有进行探测
	...
	ic = avformat_alloc_context();   //分配空间
	if (!ic) {
		print_error(filename, AVERROR(ENOMEM));
		ffmpeg_exit(1);
	}
	...
	err = av_open_input_file(&ic, filename, file_iformat, 0, ap);  // libavformant/utils.c
	if (err < 0) {
...
static void opt_output_file(const char *filename)
{
	...
	if (last_asked_format) {
		...
		file_oformat = av_guess_format(NULL, filename, NULL);  //猜测格式
		if (!file_oformat) {
...
int main(int argc, char **argv)
{
...
#endif
	av_register_all();  //注册muxer/demuxer
	...
	/* parse options */
	parse_options(argc, argv, options, opt_output_file);  //cmdutils.c
